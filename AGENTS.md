...
- 文件边界由职责与内聚性决定，这一原则适用于生产逻辑、测试、fixture、类型定义和辅助函数。围绕同一行为、共享同一上下文且通常共同变化的代码应就近放置；只有拆分确实降低理解和修改成本时才拆。
- 不设置文件行数阈值。文件较长本身不构成拆分理由；内联测试、模块门面、类型聚合、测试聚合器、协议映射和纯数据表都可能在职责单一的前提下自然增长。
- 只有在子模块具有独立职责、独立不变量、不同依赖或不同变更原因时才拆分。若只能用父模块语境解释其存在，拆分通常没有收益。
- 如果拆分后需要频繁跨文件跳转、重复导入父模块内部项，或必须同时打开多个文件才能理解一个行为，说明边界过细，应优先合并。
