...
8. `ProviderAdapter` trait + DeepSeek 和 OpenAI-compatible 两个真实 provider adapter。
...
3. DeepSeek 作为一等真实 provider adapter 接入 DeepSeek 官方 Chat Completion API；同时实现通用 `OpenAiCompatibleProvider`，用于只提供 OpenAI-compatible Chat Completions 表面的第三方 provider。
...
kuncode/
  ...
  crates/
    ...
    kuncode-provider/        # ProviderAdapter trait、DeepSeek/OpenAI-compatible 实现
...
3. `DeepSeekProvider`：调用 DeepSeek 官方 Chat Completion API；MVP 关闭 streaming；secret 可从环境变量或配置文件读取，不进 EventLog。
4. `OpenAiCompatibleProvider`：调用通用 OpenAI-compatible Chat Completions API；用于非 DeepSeek 但只提供兼容接口的 provider。
5. 本地确定性测试使用 wiremock fixture 驱动真实 provider adapter，不单独实现模拟 provider。
6. fallback 规则按总体设计 §16 实现；不支持 tool calling 直接 `ModelError::CapabilityMissing` 启动失败。
...
### Phase 3：Provider Adapter 与真实 Provider
...
2. `DeepSeekProvider`：HTTP 客户端、DeepSeek 官方 Chat Completion 请求/响应映射、thinking/reasoning metadata、错误分类、429 / 5xx 限次重试、token usage 写入 `ModelRequestMeta`。
3. `OpenAiCompatibleProvider`：HTTP 客户端、通用 OpenAI-compatible Chat Completions 请求/响应映射、错误分类和重试。
4. wiremock fixture 驱动真实 provider adapter，覆盖文本回复、tool call、错误分类和重试。
5. fallback 决策表 + 单测。
6. 最小真实 provider smoke：通过 `--features deepseek-smoke` 运行一次 DeepSeek API 调用，验证认证、模型名、tool schema 和 tool call 响应解析；默认不在 PR CI 上运行。

出口：

1. DeepSeekProvider 用录制的 HTTP fixture（`wiremock` 或 `mockito`）跑通成功路径、tool call 路径、thinking metadata 与 429 重试。
2. OpenAiCompatibleProvider 用录制的 HTTP fixture 跑通成功路径、tool call 路径与 429 重试。
3. Phase 4 agent loop dry run 使用真实 provider adapter + 本地 HTTP fixture，不引入模拟 provider。
4. Secret 不出现在事件序列化输出中（regression test）。
5. 可选 deepseek-smoke 在本地至少跑通一次，并把结果记录到 Phase 记录中；失败不阻塞普通 CI，但阻塞进入 Phase 4。
...
1. 集成测试：wiremock fixture + 真实 provider adapter 驱动一次"读文件 → 修改 → 运行测试 → 报告"，事件序列与 golden 对齐。
...
# 可选：也允许直接配置 api_key；同时存在时环境变量优先。
# api_key = "..."

# 如果目标 provider 只提供 OpenAI-compatible 接口：
...
4. `kuncode-provider/tests/deepseek_fixture.rs`：基于 `wiremock` 的 DeepSeek 录制 fixture。
5. `kuncode-provider/tests/openai_compatible_fixture.rs`：基于 `wiremock` 的通用兼容 provider fixture。
6. `kuncode-runtime/tests/agent_loop_*.rs`：wiremock fixture 驱动真实 provider adapter 的端到端剧本。
7. `kuncode-runtime/tests/completion_gate.rs`：6 条规则各一例。
8. `kuncode-runtime/tests/budget_*.rs`：3 种 hard stop 各一例。

### 9.2 Golden

1. `examples/golden/context_render/`：固定 RunState → 固定 ContextSet 输出。
2. `examples/golden/event_stream/`：固定 provider fixture → 固定事件序列。
...
1. provider key 可通过环境变量或配置文件 inline secret 读取；同时存在时环境变量优先。
2. 序列化时 `ProviderProfile` 的 credential source 只能显示为 `"<env:NAME>"` 或 `"<config:provider.default.api_key>"`，不得显示真实值。
3. `redact` MVP 实现为 best effort 字符串替换；规则：常见 token 前缀（`sk-`、`AKIA`、`ghp_` 等）+ 环境中标记为 secret 的变量值 + 配置文件 secret 值。
...
3. provider fixture 集成测试覆盖：读 → 改 → 测 → 报告的完整流程。
