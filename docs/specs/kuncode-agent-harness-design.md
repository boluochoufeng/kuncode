...
总体设计只规定 provider-neutral contract，不固定具体厂商协议。MVP 的具体落点由开发计划收窄：DeepSeek 作为一等真实 adapter 接入官方 API，OpenAI-compatible 作为独立通用 adapter 供只提供兼容接口的 provider 使用。
...
1. 真实 provider adapter 配合本地 HTTP fixture 用于 deterministic integration tests。
2. DeepSeek adapter 用于真实 smoke validation；OpenAI-compatible adapter 用于兼容接口 fixture 覆盖。
3. file read/search。
4. patch/write。
...
2. Integration tests: 本地 HTTP fixture 驱动真实 provider adapter 和 agent loop，验证工具执行、事件、context、completion gate。
