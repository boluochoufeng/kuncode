...
use crate::{
    session_store::{
        ...
        SessionStoreError, active_messages_sha256, turso::TursoSessionStore,
    },
...
async fn compaction_rejects_non_canonical_sha256_hashes() {
    ...
    let checkpoint = checkpoint_with_text(&session, source, "summary");
    ...
    for (input_hash, output_hash) in [
        ...
        let result = store
            .commit_compaction(NewCompactionCommit {
                session_id: session.clone(),
                expected_journal_head: source,
                event: hash_event(input_hash, output_hash, source),
...
async fn compaction_rejects_output_hash_for_different_active_messages() {
    let (store, session, source) = store_with_source().await;
    let checkpoint = checkpoint_with_text(&session, source, "installed summary");
    ...
    let result = store
        .commit_compaction(NewCompactionCommit {
            session_id: session.clone(),
            expected_journal_head: source,
            event: hash_event(VALID_INPUT_HASH, wrong_output, source),
            checkpoint,
        })
        .await;
    let entries = store
